...
| D12 | Provider management surface | Global settings sheet · provider detail pages · per-card popovers | ✅ **Reverted: global settings sheet** | The dashboard/provider-page experiment was rolled back. Keep the v0.1 compact tray-panel structure for now: provider setup and connection controls live in Settings, while the main panel stays focused on provider cards. Revisit dashboard IA only after a better design structure is decided. |
...
| D15 | Tray placement | Always tray-attached · floating/popped-out mode · separate pinned window | Restored: **v0.1 tray popup with floating position** | The header pin/pop-out control is preserved in the rollback, but it controls whether the tray popup snaps near the tray or keeps its floating position. The main popup remains a tray-style always-on-top utility window; the separate glance window remains draggable and always-on-top. |
...
| D19 | Glance surface | Tray tooltip/badge · draggable glance widget · taskbar overlay/native shell extension | ✅ **Decided: draggable always-on-top glance widget** | Windows taskbar overlay behavior is too brittle for the long run. Keep the glance surface as a small draggable always-on-top window with remembered position. It shows compact `current | weekly` style values for the default glance providers, with three-value display where needed for Claude Fable and Antigravity pools. The tray icon opens the main panel. |

| D20 | Main window direction | Fixed tray popup with Focus/Dashboard toggle - resizable dashboard app - separate compact/full windows | Paused: **dashboard redesign rolled back** | The first resizable dashboard/sidebar experiment was useful but not good enough to keep. v0.1.1 restores the compact tray-panel structure while preserving glance, Antigravity, Claude Fable 5, and Codex reset credits. Revisit the dashboard design with a stronger layout direction before rebuilding it. |
...
| D23 | Dashboard completion order | More providers first - dashboard first - structured metrics first | Superseded: **dashboard redesign paused** | PR #5 completed the Antigravity/provider-expansion slice, but the first dashboard/sidebar structure did not work well enough. Roll back to the v0.1 tray-panel structure plus glance, while preserving Codex reset credits, Claude Fable 5, and Antigravity. Revisit dashboard IA after a stronger design direction is chosen. |
...
- [x] Codex reset banks: show available reset credits and sorted expiry dates when the dedicated endpoint is available.
