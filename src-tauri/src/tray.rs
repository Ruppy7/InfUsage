...
pub fn set_display_mode(app: &tauri::AppHandle, mode: &str) -> tauri::Result<()> {
    ...
    show_popup_window_for_size(&window, width, height);
    Ok(())
}
...
fn show_popup_window_for_size(window: &tauri::WebviewWindow, width: f64, height: f64) {
    let popped_out = POPPED_OUT.load(Ordering::Relaxed);
    if !popped_out {
        position_near_bottom_right_for_size(window, width, height);
    }
    show_window(window);
}
...
fn position_near_bottom_right_for_size(window: &tauri::WebviewWindow, width: f64, height: f64) {
    let Ok(Some(monitor)) = window.current_monitor() else {
        return;
    };
    ...
    let work_area = monitor.work_area();
    let x = work_area.position.x + work_area.size.width as i32 - target_width - POPUP_MARGIN;
    let y = work_area.position.y + work_area.size.height as i32 - target_height - POPUP_MARGIN;
