...
const LOCAL_LOGIN_PROVIDERS: ProviderKey[] = ["antigravity", "codex", "claude"];
...
function readDisconnectedProviders(): DisconnectedProviders {
  ...
  try {
    ...
    return LOCAL_LOGIN_PROVIDERS.reduce<DisconnectedProviders>((next, providerId) => {
      if (parsed[providerId] === true) next[providerId] = true;
      return next;
    }, {});
  } catch {
    return {};
...
function App() {
  ...
  useEffect(() => {
    try {
      localStorage.removeItem("limitlens.dashboardView");
      localStorage.removeItem("limitlens.starredProviders");
    } catch {
      // Storage can be unavailable in restricted webviews; cleanup is best-effort.
    }
  ...
  function cardFor(key: ProviderKey): ReactNode {
    ...
    const planLabel = key === "antigravity" || key === "codex" || key === "claude" ? rawMetrics.find((line) => line.label === "Plan")?.value : undefined;
    ...
    >
      <header className="panel-header" onPointerDown={startWindowDrag}>
        ...
        <div className="header-actions">
          ...
          <button
            aria-label={poppedOut ? "Floating position - attach near tray" : "Attached position - float window"}
            aria-pressed={poppedOut}
            className="icon-button pin-button"
            onClick={() => setPoppedOut((current) => !current)}
            title={poppedOut ? "Floating position" : "Attached near tray"}
            type="button"
          >
...
function GlanceApp() {
  ...
  const items = [
    ...
    glanceItem("claude", anthropicIcon, snapshots.claude, "Session", "Weekly", "Fable 5"),
    glanceItem("opencode", opencodeIcon, snapshots.opencode, "Rolling", "Weekly"),
    glanceAntigravityItem("antigravity", antigravityIcon, snapshots.antigravity),
  ];
...
function MetricRow({ line }: { line: MetricLine }) {
  const resetCredits = resetCreditsParts(line);
  if (resetCredits) {
    return (
      <div className="metric-row reset-credit-row">
        <div className="reset-credit-line">
          <span className="reset-credit-main">
            Resets: <strong>{resetCredits.count}</strong>
          </span>
          {resetCredits.expiryDates.length > 0 && (
            <span className="reset-credit-expiry">
              <Clock aria-hidden="true" size={12} />
              <span>[{resetCredits.expiryDates.join(", ")}]</span>
            </span>
          )}
        </div>
      </div>
...
function resetCreditsParts(line: MetricLine) {
  if (line.label !== "Resets available") return null;

  const count = line.value.match(/^\s*(\d+)/)?.[1];
  if (!count) return null;

  const expiryDates = line.value
    .match(/\(([^)]+)\)/)?.[1]
    ?.split(",")
    .map((date) => {
      const match = date.trim().match(/^(\d{1,2})-([A-Za-z]{3})-\d{2}$/);
      if (!match) return date.trim();
      return `${match[1].padStart(2, "0")}-${match[2]}`;
    })
    .filter(Boolean) ?? [];

  return { count, expiryDates };
}
